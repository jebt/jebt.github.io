const sketch = function(p) {
  ...
  function changePixel() {
    ...
    dark2 = Math.floor(p.dist(x, y, p.width/2, p.height/2));
    bright = 150 - Math.floor(p.dist(x, y, (p.width/2), (p.height/2)));
    ...
    disco = p.color(Math.floor(R - (dark + dark2) + bright), Math.floor(G - (dark + dark2) + bright), Math.floor(B - (dark + dark2) + bright), A);
  ...
  p.setup = function() {
    ...
    for (let i = 0; i < img.pixels.length; i++) {
      img.pixels[i] = p.color(0);
    }
    img.updatePixels();
    p.image(img, 0, 0);
    grid = new Array(p.width).fill().map(() => new Array(p.height).fill(0));
    x = p.width / 2;
    y = p.height / 2;
    dir = NORTH;
  };
